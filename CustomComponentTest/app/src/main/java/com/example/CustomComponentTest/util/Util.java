package com.example.CustomComponentTest.util;
    ...
    * */
    public static Bitmap createQRCode(int width, int height, String source) {
        try {
            ...
            BitMatrix bitMatrix = new QRCodeWriter().encode(source, BarcodeFormat.QR_CODE, width, height, hints);//将信息编码成位矩阵

            int[] pixels = new int[width * height];//将位矩阵转化为一维数组中
